...
- scheduler/executor process heartbeats now write through O_NOFOLLOW descriptors, force existing files to mode 0600, and enforce a bounded write deadline; API boolean parsing accepts normalized operator input and artifact byte defaults/ceilings are named separately
...
- executor transports now cancel unused response bodies, reject webhook redirects, retain schema mismatch causes without reflecting payloads, abort active handlers on process shutdown, fail unsupported kinds without futile retries, and bound debug-proxy upstream requests
...
- the Reports workspace now polls only the submitted Browser Audit resource, cancels polling on a replacement submission or route teardown, and performs full control-data refreshes only when the queued item is first registered or reaches a terminal state
