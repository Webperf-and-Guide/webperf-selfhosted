...
Brand:
- brand/product: `WebPerf & Guide`
- managed edition: `WebPerf & Guide Managed`
- managed service domain: `webperf.and.guide`
- public OSS edition/repo: `WebPerf Self-hosted` / `webperf-selfhosted`
...
Current repo state as of 2026-08-02:
- direct release-bundle upgrades are supported from `v0.4.0`, the first public release of the final consolidated three-image/two-container default topology; the split-role `v0.2.x` GHCR packages are intentionally retired and are not release dependencies
- backup, restore, database-doctor, and cross-version Compose drills are automated; the recovery drill verifies checksums and restored rows, while versions newer than `v0.4.0` must stop the checksum-verified public baseline writers, create and verify an explicit SQLite backup, replace the baseline on the same named volume, preserve Site/Route Group/Check/completed Job data and provenance, pass database doctor, and complete a post-upgrade Check run
- Compose database and artifact maintenance commands explicitly run as UID/GID `1000:1000`, matching the API-owned `/data` volume instead of inheriting the standalone container's root supervisor identity
...
- the standalone supervisor starts the API and console independently of probe health, then waits for the separate probe `/healthz` endpoint before starting the executor so stack startup races leave durable jobs queued instead of consuming their bounded attempts
...
- pushes to `main` publish all three runtime images with `main` and `sha-*` development tags after required CI; tagged releases publish version tags, digest-pinned Compose, SPDX SBOMs, provenance, checksums, and release-scoped runtime metadata without a `latest` channel
...
- the Rust probe wire contract now omits an absent custom request configuration, rejects explicit JSON `null` to match the public Zod schema, and follows the documented four redirects before rejecting a fifth; custom-Compose upgrade guidance also names the standalone supervisor's exact identity and capability boundary
...
- public-beta tags are accepted only from `main` after Sampo changesets are applied, the tag matches the highest public package version, and the protected `release` environment approves publication; release assets are generated deterministically from three same-commit image digests and attested in GitHub and GHCR
...
- protected release runs `30161891204` and `30191805871` historically published annotated tags `v0.2.0` and `v0.2.1`; those six-image bundles are superseded, while the current `v0.4.0` release publishes the three active multi-architecture GHCR images, a digest-pinned Compose bundle, per-platform SPDX SBOMs, provenance attestations, checksums, and release-scoped runtime metadata
...
- the three active GHCR packages (`webperf`, `webperf-probe`, and `webperf-browser-audit-lighthouse`) are public; the retired split-role and worker packages are absent by design
- formal releases finish with a required fresh GitHub-hosted published-bundle smoke for both default and Browser Audit profiles; its bundle-aware harness is separate from a source-pinned checkout that must match the bundle runtime metadata, and releases newer than `v0.4.0` additionally run the supported same-volume upgrade drill
...
- issue #14 Phase 2+3 is complete: the four Bun runtime images (`webperf-console`, `webperf-api`, `webperf-scheduler`, `webperf-executor`) are consolidated into a single multi-role `webperf` image built from `infra/docker/Dockerfile.webperf`. The active role is selected at container start by the `WEBPERF_ROLE` environment variable (`console`, `api`, `scheduler`, or `executor`) via the `tooling/scripts/webperf-role.ts` dispatcher. The scheduler defaults to embedded mode inside the API process (`SELFHOST_SCHEDULER_MODE=embedded`); external mode remains available. The published GHCR image set changed from six images to three (`webperf`, `webperf-probe`, `webperf-browser-audit-lighthouse`); `infra/docker/README.md`, `docs/quickstart/runtime-images.md`, `infra/release/README.md`, `docs/contributors/releases.md`, `docs/users/install.md`, `docs/users/upgrade.md`, `docs/architecture/execution-model.md`, `CONTRIBUTING.md`, and this file document the three-image model. `v0.4.0` is the supported direct-upgrade baseline, so formal releases no longer depend on the retired split-role packages.
- the superseded Regional Runtime API and deployment profile have been removed. WebPerf & Guide Managed consumes the stateless `webperf-probe` image directly and privately owns provider application IDs, deploy/undeploy lifecycle, queues, capacity policy, global fan-out, and aggregation. The full self-host remains an independent single-location product. Probe request buffering now uses a separate fixed memory budget and bounded body-read deadline, lifecycle timeouts preserve the last validated redirect URL and count for diagnostics, and the self-host executor validates echoed correlation identifiers without dropping legacy responses that omit them.
- standalone upgrade and recovery guidance preserves the consolidated `webperf` service origins and stages restore databases as UID/GID `1000`; the cross-version drill starts the public `v0.4.0` baseline and separate probe before verifying the candidate against the retained volume.
...
1. complete the protected `v0.5.0` release, then confirm the three public version-tagged images, digest-pinned bundle, and supported `v0.4.0` upgrade drill
