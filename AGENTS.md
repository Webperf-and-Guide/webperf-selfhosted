...
- `ci-selfhost` now validates Compose through the checked-in `compose:config` script, which uses `infra/docker-compose/.env.example` instead of assuming repository-local secrets, requires bounded tmpfs mounts, and distinguishes unavailable Docker from a definite render failure
- local Browser Audit artifact operations now pin validated, process-owned audit directories across downloads and POSIX descriptor-relative deletes, cache the native unlink binding, reject directory identity changes, and document the API-only Compose volume boundary
...
- the Reports Browser Audit controller now runtime-validates create and polling payloads, keeps a queued audit distinct from a failed initial refresh, bounds displayed API errors, caps individual status requests and consecutive 5xx checks, and tells operators to refresh after the overall polling deadline
...
- artifact publication now rejects duplicate filesystem identities without replacing bytes, persisted artifact kinds and execution availability are bounded by shared contracts, stale response digests are removed after redaction, and configured webhook signing secrets require at least 16 characters
- the Lighthouse runner now URL-encodes every artifact upload path/query value, cancels rejected upload bodies, models user-flow payloads without `any`, resumes short-secret redaction after unmatched quotes, and closes CONNECT work across DNS/client races while bounding authority and HTTP framing inputs
...
- the Lighthouse image now pins arm64 Chromium to the Debian security snapshot matching the locked Puppeteer revision, and every artifact upload consumes only the remaining shared audit deadline (also bounded by upload-token expiry) instead of starting a fresh total timeout
...
- scheduler/executor process heartbeats now publish same-directory O_NOFOLLOW/O_EXCL private temporary files through fsync plus atomic rename, clean failed publications, keep owner-only permissions, write epoch milliseconds, enforce a bounded write deadline, and contain synchronous or asynchronous logger failures behind generic diagnostics; API boolean parsing accepts normalized operator input and artifact byte defaults/ceilings are named separately
...
- API validation now distinguishes missing JSON bodies, constrains extension provider IDs to safe identifiers, fails closed on locked redaction streams, and records bounded diagnostics for encrypted-row decode failures; artifact index limits are checked transactionally with the SQLite trigger retained as a cross-process backstop
...
- local artifact reads and directory permission changes now stay bound to `O_NOFOLLOW` file descriptors, upload-grant validation names every bounded claim while documenting nonce semantics, and SQLite payload crypto rejects weak secrets at its own trust boundary
