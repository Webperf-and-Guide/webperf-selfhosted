...
- scheduler/executor process heartbeats now publish same-directory O_NOFOLLOW/O_EXCL private temporary files through fsync plus atomic rename, clean failed publications, keep owner-only permissions, enforce a bounded write deadline, and report logger failures with generic diagnostics only; API boolean parsing accepts normalized operator input and artifact byte defaults/ceilings are named separately
