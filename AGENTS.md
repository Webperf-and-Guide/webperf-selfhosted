...
- the Reports Browser Audit controller now runtime-validates create and polling payloads, keeps a queued audit distinct from a failed initial refresh, bounds displayed API errors, and caps consecutive 5xx status checks alongside the deadline-aware backoff
