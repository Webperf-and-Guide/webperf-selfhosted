...
- artifact reconciliation pins the private root and every audit directory while enumerating and deleting descriptor-relative entries, classifies swapped root entries by the entry actually removed without following symlinks, and persisted registry validation consumes the public contract version constant instead of duplicating it
