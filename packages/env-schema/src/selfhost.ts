...
export const selfhostControlEnvSchema = z.object({
  SELFHOST_CONTROL_HOST: z.string().min(1).default('0.0.0.0'),
  SELFHOST_CONTROL_PORT: z.preprocess(
    (value) => value ?? '8788',
    z.coerce.number().int().positive()
  ),
  SELFHOST_DATABASE_PATH: z.string().min(1).default('./data/control.sqlite'),
...
export const parseSelfhostControlVars = (
  input: Partial<Record<keyof z.infer<typeof selfhostControlEnvSchema>, string | number | undefined>>
) =>
  selfhostControlEnvSchema.parse({
    SELFHOST_CONTROL_HOST: input.SELFHOST_CONTROL_HOST,
    SELFHOST_CONTROL_PORT: input.SELFHOST_CONTROL_PORT,
