...
import { parseSelfhostControlVars } from '@webperf/env-schema/selfhost';
import {
...
function parseRuntime(input: Record<string, string | undefined>): SelfhostRuntime {
  const parsed = parseSelfhostControlVars(input);

  return {
    host: parsed.SELFHOST_CONTROL_HOST,
    port: parsed.SELFHOST_CONTROL_PORT,
  ...
  if (!controlOpenApiDocumentPromise) {
    controlOpenApiDocumentPromise = Promise.resolve(
      buildControlOpenApiDocument({
        ...
        version: '0.1.0',
        description:
          'Contract-first API for the self-hosted Webperf control service. `/v1/*` remains the stable REST surface, while `/rpc/*` exposes the same procedures over oRPC.',
        serverUrl: '/'
      })
    );
  }
