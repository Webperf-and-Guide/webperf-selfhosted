...
describe('control service monitoring expansion', () => {
  test(
    ...
    async () => {
      ...
      expect(health.service).toBe('webperf-control');
      expect(health.ok).toBe(true);
      expect((await opsClient.system.health()).service).toBe('webperf-control');
...
const createTempDirectory = () => {
  const directory = mkdtempSync(join(tmpdir(), 'webperf-control-http-'));
...
const startSelfhostHarness = async (probePort: number) => {
  ...
  process.env.SELFHOST_CONTROL_HOST = '127.0.0.1';
  process.env.SELFHOST_CONTROL_PORT = `${controlPort}`;
