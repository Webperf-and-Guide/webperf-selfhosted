...
- PRs #4 through #11 closed the local Compose port-isolation, canonical baseline/deterministic analysis, multi-architecture release, release-bundle-version, and fresh-host runtime evidence gaps; the public-beta hardening plan records every verified gate
- all six GHCR packages are public, and anonymous registry checks verify `0.2.1` manifests for both `linux/amd64` and `linux/arm64`; GitHub-hosted published-bundle smoke run `30196392990` verified the archive checksums, source metadata, empty Docker credential directory, and both runtime profiles
- formal releases now finish with a required fresh GitHub-hosted published-bundle smoke for both default and Browser Audit profiles; its current bundle-aware harness is kept separate from a source-pinned checkout that must match the bundle runtime metadata, and run `30196392990` completed that historical `v0.2.1` drill
