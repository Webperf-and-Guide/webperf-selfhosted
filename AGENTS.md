...
- the Lighthouse image keeps packaged sandbox helpers root-owned but non-setuid, explicitly selects the user-namespace sandbox, and relies on its product-specific Chrome path plus the checked-in seccomp policy instead of a privileged helper
...
- the amd64 Lighthouse image places that pinned browser under `/opt/webperf/chrome` to avoid Ubuntu host AppArmor profiles aimed at conventional Chrome paths, while production Compose applies a Moby `seccomp/v0.2.1`-based default-deny profile with only Chromium's `clone`, `setns`, and `unshare` namespace operations added; the worker remains non-root and receives neither `SYS_ADMIN` nor `--no-sandbox`
