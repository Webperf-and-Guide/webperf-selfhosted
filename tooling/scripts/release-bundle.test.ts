...
`
    ...
    expect(pullRequest.body).toContain(
      'the protected release workflow will publish the GHCR images and install bundle for `v0.3.0`'
    );
    expect(pullRequest.body).toContain(
      'npm package publication remains a separate future concern.'
    );
    expect(pullRequest.body).not.toContain('Registry publication remains separate');
